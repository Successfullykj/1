...
bot = telebot.TeleBot('7674417277:AAGMvXZAUCsJUztq1lANS8R00DVoO404SEU')
...
def is_user_in_channel(user_id):
    try:
        ...
        return False
@bot.message_handler(commands=['MISSYOU'])
def handle_attack(message):
    ...
    if is_attack_running(user_id):
        bot.reply_to(message, "DEAR PAID USER 1 ATTACK IS ALREADY PROGRESS PLEASE WAIT FEW MINUTES")
        return
    ...
    if len(command) != 4:
        bot.reply_to(message, "USAGE = /MISSYOU <IP> <PORT> <TIME>")
        return
    ...
    if time_duration > 240:
        bot.reply_to(message, "240 SECOND MAX ATTACK TIME ISSE JADA NHI HOGA SORRY")
        return
...
def handle_screenshot(message):
    ...
    if pending_feedback.get(user_id, False):
        ...
        bot.reply_to(message, "FEEDBACK ACCEPTED THANKS FOR SENDING FEEDBACK")
    else:
        bot.reply_to(message, "THIS IS NOT VALID RESPONSE ")
@bot.message_handler(commands=['start'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f"""PAID GROUP PE WC {user_name}
